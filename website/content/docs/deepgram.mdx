...
The Deepgram provider provides a `tts` function that allows you to create a text-to-speech synthesis function using Deepgram TTS. By default, the `tts` function uses the `aura-asteria-en` model.
...
The Deepgram provider provides Typescript definitions for the `tts` and `stt` functions.
